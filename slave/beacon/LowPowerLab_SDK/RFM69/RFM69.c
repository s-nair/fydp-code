...
void sendFrame(uint8_t toAddress, const void* buffer, uint8_t bufferSize, bool requestACK, bool sendACK)
{
    ...
    SPI_transfer_byte(bufferSize + 3);
    SPI_transfer_byte(toAddress);
    SPI_transfer_byte(_address);
    SPI_transfer_byte(CTLbyte);
...
uint8_t readReg(uint8_t addr)
{
    ...
    uint8_t regval = SPI_transfer_byte(0);
    unselect();
    return regval;
}
...
//    Serial_print("Address - HEX - BIN");
    ...
    {
        select();
        SPI_transfer_byte(regAddr & 0x7F); // send address + r/w bit
        regVal = SPI_transfer_byte(0);
        unselect();
